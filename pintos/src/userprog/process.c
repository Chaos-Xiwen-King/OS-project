...
//把分割的文件名传递给thread_create。
  ...
  sema_down(&thread_current()->child_load_finished);
  if (!thread_current()->child_load_success) return TID_ERROR;
...
//源pintos文件没有关于当内存分配成功(success==true)时候的操作，需要按照3.5.1中的描述实现。
  ...
  //分割file_name的拷贝并且入栈
    ...
    *(int *)if_.esp = 0;
    ...
    sema_up (&thread_current ()->parent->child_load_finished);
  }
...
process_wait (tid_t child_tid)
{
  ...
  child_elem = list_begin (childs);
  struct child *child = NULL;
  while (child_elem != list_end (childs))
  {
    child = list_entry (child_elem, struct child, child_elem);
    if (child->tid == child_tid)
    {
      if (!child->have_been_wait)
      {
        child->have_been_wait = true;
        sema_down (&child->finished);
        break;
      } 
      else 
      {
